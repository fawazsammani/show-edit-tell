...
def save_checkpoint(epoch, epochs_since_improvement, dae, dae_optimizer, cider, is_best):
...
class DAE(nn.Module):
    ...
    def forward(self, encoded_captions, caption_lengths, encoded_previous_captions, previous_cap_length):
        ...
        # Previous caption mask is assigned in the Caption Encoder
...
def train(train_loader, dae, criterion, dae_optimizer, epoch):

    dae.train()  # train mode (dropout and batchnorm is used)
    ...
    for i, (caption, caplen, previous_caption, prev_caplen) in enumerate(train_loader):
        ...
        # Forward prop.
        scores, caps_sorted, decode_lengths, sort_ind = dae(caps, caplens, previous_caption, prev_caplen)
        ...
        torch.nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, dae.parameters()), 0.25)
...
def evaluate(loader, dae, beam_size, epoch, word_map):
    ...
    for i, (image_id, previous_caption, prev_caplen) in enumerate(tqdm(loader, desc="EVALUATING AT BEAM SIZE " + str(beam_size))):
        ...
        previous_encoded, final_hidden, prev_caption_mask = dae.caption_encoder(encoded_previous_captions, prev_caplen)
        ...
        # Start decoding
        step = 1
        h1, c1 = dae.init_hidden_state(k)  # (batch_size, decoder_dim)
        h2, c2 = dae.init_hidden_state(k)

        # s is a number less than or equal to k, because sequences are removed from this process once they hit <end>
        while True:

            embeddings = dae.embed(k_prev_words).squeeze(1)        
            topdown_input = torch.cat([embeddings, final_hidden, h2],dim=1)
            h1,c1 = dae.attention_lstm(topdown_input, (h1, c1))
            attend_cap = dae.caption_attention(previous_encoded, h1, prev_cap_mask)
            language_input = torch.cat([h1, attend_cap], dim = 1)
            h2,c2 = dae.language_lstm(language_input, (h2, c2))
            scores = dae.fc(h2)  
...
if checkpoint is None:
    dae = DAE(word_map, emb_file)
    dae_optimizer = torch.optim.Adam(params=dae.parameters(),lr=dae_lr)
    ...
    dae = checkpoint['dae']
    dae_optimizer = checkpoint['dae_optimizer']
...
for epoch in range(start_epoch, epochs):
    ...
    # Save checkpoint
    save_checkpoint(epoch, epochs_since_improvement, dae, dae_optimizer, recent_cider, is_best)
